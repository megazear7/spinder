...
### Shared Types
 ...
 - Types must be imported by the client code and used in components, providers, pages, utils, and all other code.
 - Types must be imported by the server code and used in controllers, services, utils, prompts, and all other code.

### Shared Routes
 - All page routes are defined in the `RouteConfig` type in `src/shared/service.client.ts`.
...
### Shared Utilities
 ...
 - Shared utilities must not rely on Browser APIs so that they can be reused in any context.
